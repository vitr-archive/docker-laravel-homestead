...
apt-get install -y --allow-unauthenticated build-essential dos2unix gcc git libmcrypt4 libpcre3-dev ntp unzip \
make python2.7-dev python-pip re2c supervisor unattended-upgrades whois vim libnotify-bin \
pv cifs-utils mcrypt mc
...
printf "\nPATH=\"$(su - vagrant -c 'composer config -g home 2>/dev/null')/vendor/bin:\$PATH\"\n" | tee -a /home/vagrant/.profile
...
ln -sf /home/vagrant/.composer/vendor/laravel/installer/laravel /usr/local/bin/laravel
...
apt-get install -y --allow-unauthenticated postgresql
...
apt-get install -y --allow-unauthenticated blackfire-agent blackfire-php
